...
namespace Concentrator.Configuration.Core
{
  public class Environments
  {
    ...
    private static EnvironmentConfigSection ConfigSection
    {
      get
      {
        ...
        FileInfo fInf = new FileInfo(Assembly.GetExecutingAssembly().Location);

        var configFile = Path.Combine(fInf.Directory.FullName, "Environment.config");

        if (!File.Exists(configFile)) throw new FileNotFoundException("Environment.config file is missing");

        var map = new ExeConfigurationFileMap()
        {
          ExeConfigFilename = configFile
        };
        var _config = ConfigurationManager.OpenMappedExeConfiguration(map, ConfigurationUserLevel.None);

        return _configSection ?? (_configSection = (EnvironmentConfigSection)_config.GetSection("Environment"));
      }
    }
  }
}