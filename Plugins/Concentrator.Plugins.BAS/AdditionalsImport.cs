...
{
  public class AdditionalsImport : ConcentratorPlugin
  {
    public override string Name
    {
      get { return "BAS FreeGoods and Accruals Import Plugin"; }
    }
    ...
    protected override void Process()
    {
      foreach (Vendor vendor in Vendors.Where(x => ((VendorType)x.VendorType).Has(VendorType.Assortment) && x.IsActive))
      {
        ...
        log.DebugFormat("Start FreeGoods and Accruals Import for Vendor '{0} ({1})'", vendor.Name, vendor.VendorID);
        ...
        var ass = new ProcessAdditionals();
        ...
        log.DebugFormat("Finished FreeGoods and Accruals Import for vendor '{0} ({1})'", vendor.Name, vendor.VendorID);
