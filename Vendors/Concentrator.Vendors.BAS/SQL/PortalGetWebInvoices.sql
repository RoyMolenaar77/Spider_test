/****** Object:  StoredProcedure [dbo].[PortalGetWebInvoices]    Script Date: 03/07/2011 12:10:16 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[PortalGetWebInvoices]
	(	
@CustomerID int,
@MaxRows int = null,
...
SELECT	
                      ...
                      SDVR02 AS WebOrderNumber, SDPTC AS PaymentTermsCode, (SELECT DRDL01
FROM         F0005
WHERE     (DRSY = '00') AND (DRRT = 'PY') and ltrim(rtrim(drky)) = ltrim(rtrim(rpryin))) AS PaymentInstrument,
                      ...
                      DropAddress.OACTR AS DropAddressCountry, rpcrcd as CurrencyCode
                      ,RPDOC,
		(dbo.JDEToDate(RPDIVJ)) AS invoiceDate,
		(dbo.JDEToDate(RPDDJ)) AS InvoiceExpireDate,amountgross,AmountOpen,AmountTaxable,AmtTax,
...
F42119	
INNER JOIN (
--select RPDOC,RPDIVJ,RPDDJ,RPCRRM,RPACR,RPAG,RPFAP,RPAAP,RPDCT,RPKCO,RPCRCD
--FROM F03B11
--WHERE  (
--			(RPDCT = 'RI')
--			OR
...
--            (RPDCT = 'RM')
--		)
--		AND (RPAN8 = @SoldToNumber) 
--) F03B11 on (RPDOC = SDDOC AND RPDCT = SDDCT AND RPKCO = SDKCO)select * from F03B11
 SELECT     RPDOC, RPDCT,RPKCO, RPAG / 100 AS AmountGross, 
                                                  RPAAP / 100 AS AmountOpen, RPATXA / 100 AS AmountTaxable, 
                                                 RPSTAM / 100 AS AmtTax, RPDDJ, RPDIVJ,RPCRRM,rpcrcd, rpryin
                            FROM  F03B11) F03B11 on (RPDOC = SDDOC AND RPDCT = SDDCT AND RPKCO = SDKCO)
INNER JOIN
...
where SDAN8 = @SoldToNumber
		...
		and (@SearchNumber is null or (@SearchNumber is not null and (sddoco = @SearchNumber or RPDOC = @SearchNumber)))
GROUP BY RPDOC,RPDCT,RPCRRM, RPDIVJ, RPDDJ,SDDOCO,
 SDKCOO, SDDOCO, SDDCTO, SDMCU , SDAN8, SDSHAN, 
                      ...
                      DropAddress.OACTR ,amountgross,AmountOpen,AmountTaxable,AmtTax,rpcrcd,rpryin,soldtoname.abalph,shiptoname.abalph
HAVING AmountOpen <> 0 
END
else
BEGIN
...
--		)
--		AND (RPAN8 = @SoldToNumber) 
--) F03B11 on (RPDOC = SDDOC AND RPDCT = SDDCT AND RPKCO = SDKCO)
 SELECT     RPDOC, RPDCT,RPKCO, RPAG / 100 AS AmountGross, 
                                                  RPAAP / 100 AS AmountOpen, RPATXA / 100 AS AmountTaxable, 
                                                 RPSTAM / 100 AS AmtTax, RPDDJ, RPDIVJ,RPCRRM,rpcrcd,rpryin
                            FROM  F03B11) F03B11 on (RPDOC = SDDOC AND RPDCT = SDDCT AND RPKCO = SDKCO)
