/****** Object:  StoredProcedure [dbo].[PortalGetWebOrderLines]    Script Date: 03/07/2011 12:10:32 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[PortalGetWebOrderLines] 
	(
@OrderID int,
@CustomerID int,
@OrderType char(2)		
...
FROM         (SELECT   SHKCOO, SHDOCO, SHDCTO, SHMCU, SHAN8, SHSHAN, SHPA8, SHVR01, SHVR02, SHPTC, SHRYIN, SHTXA1, SHEXR1, SHCARS, SHROUT, 
                       ...
                       FROM         F4201) AS OrderHeader INNER JOIN
                          (SELECT SDKCOO, SDDOCO, SDDCTO, SDLNID, SDMCU, SDTRDJ, SDIVD, SDCNDJ, SDVR01, SDVR02, SDITM, SDLITM, SDAITM, SDDSC1, 
                            ...
                            FROM          F4211) AS OrderDetail ON OrderHeader.SHKCOO = OrderDetail.SDKCOO AND 
...
where (OrderHeader.SHAN8 = @CustomerID) 
	AND (OrderDetail.SDDOCO = @OrderID) 
	AND(OrderDetail.SDDCTO = @OrderType) 
	AND (OrderDetail.SDKCOO = @companyCode)

--select * from f4211
--inner join f4201 on sdkcoo = shkcoo and sddoco = shdoco and sddcto = shdcto
--where sddoco = 10171034
--and shan8 = 18750000
--and sddcto = 'SZ'
...
--	   F4211.SDDSC2, 
--	   F4211.SDSOQS, 
--	   F4211.SDUPRC / 10000 AS UPRC, 
--	   F4211.SDAEXP / 100 AS AEXP,
--       F4211.SDTAX1, 
--       F4211.SDTXA1, 
--       F4211.SDCRCD, 
--       F4211.SDCRR, 
--       F4211.SDFUP / 10000 AS FUP, 
--       F4211.SDFEA / 100 AS FEA, 
--       (F4008.TATXR1+F4008.TATXR2) / 1000 AS Vat, 
--       F4211.SDKCOO, 
--       F4211.SDDOCO, 
--       F4211.SDDCTO, 
--       F4211.SDAITM
--FROM F4211 INNER JOIN
--     F4008 ON (F4211.SDTXA1 = F4008.TATXA1 AND CASE F4211.SDTAX1 WHEN 'y' THEN 5 WHEN 'n' THEN 3 ELSE F4211.SDTAX1 END = F4008.TAITM)
--WHERE (F4211.SDAN8 = @CustomerID) 
--	AND (F4211.SDDOC = @InvoiceID) 
--	AND(F4211.SDDCT = @InvoiceType) 
--	AND (F4211.SDKCO = @companyCode)
