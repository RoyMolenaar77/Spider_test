...
namespace Concentrator.Vendors.BAS.Web.Services
{
  ...
  public class GetInvoiceLines : IHttpHandler
  {
    ...
    public void ProcessRequest(HttpContext context)
    {
      ...
      if (string.IsNullOrEmpty(context.Request.Params["InvoiceNumber"]))
      {
        context.Response.Write("Invalid Invoice Number");
        return;
      }

      if (string.IsNullOrEmpty(context.Request.Params["InvoiceType"]))
      {
        context.Response.Write("Invalid Invoice Type");
        return;
      }
      ...
      using (StringWriter sw = new StringWriter())
      {
        ...
        using (DbCommand command = db.GetStoredProcCommand("dbo.PortalGetWebInvoiceLines"))
        {
          db.AddInParameter(command, "InvoiceID", DbType.Int32, context.Request.Params["InvoiceNumber"]);
          db.AddInParameter(command, "CustomerID", DbType.Int32, context.Request.Params["CustomerID"]);
          db.AddInParameter(command, "InvoiceType", DbType.String, context.Request.Params["InvoiceType"]);
