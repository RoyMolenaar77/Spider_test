/****** Object:  View [dbo].[MissingContentView]    Script Date: 02/17/2011 13:36:17 ******/
SET ANSI_NULLS ON
GO
...
b.Name as brandName,
c.ShortDescription,
cast(case when (Select COUNT(*) from ProductMedia pm
			inner join MediaType mt on pm.TypeID = mt.TypeID
			where ProductID = c.ProductID and mt.Type = 'image') > 0 then 1 else 0 end as bit)as [Image],
cast(case when (Select COUNT(*) from ProductMedia pm
			inner join MediaType mt on pm.TypeID = mt.TypeID
			where ProductID = c.ProductID and mt.Type = 'youtube' and Sequence = 0) > 0 then 1 else 0 end as bit) as youtube,
cast(case when (Select COUNT(*) from ProductAttributeMetaData pamd
			...
			where pamd.VendorID = cvs.VendorID and pav.ProductID = c.ProductID) > 0  then 1 else 0 end as bit) as Specifications,
