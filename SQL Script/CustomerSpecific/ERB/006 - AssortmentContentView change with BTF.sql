...
SELECT distinct
      ...
      p.Visible,
			...
			left join ConcentratorDescriptionCount pd on pd.productid = c.productid
						...
						left join ProductAttributeValue pavBtF on pavBtf.productid = p.productid and pavBtF.attributeid in
								(select attributeid from productattributemetadata where attributecode = 'btf')
		...
		(p.IsNonAssortmentItem = 1 or 
			(
				(
					p.isconfigurable = 1 or 
					(p.IsConfigurable = 0 and rp.ProductID is null) or
					(p.IsConfigurable = 0 and rp.ProductID is not null and cConfi.ProductID is not null)
				)
				
				and 
				(ct.IgnoreMissingImage is null or (ct.IgnoreMissingImage = 1 and pm.imageCount > 0 and (rp.productid is null or (rp.productid is not null and cpm.imageCount > 0)))) 
				
				and
				(
					(ct.IgnoreMissingConcentratorDescription = 0 or  
						(	(p.IsConfigurable = 1 and pd.ProductID is not null) or 
							(p.IsConfigurable = 0 and pd.productid is not null and rp.RelatedProductID is null) or 
							(p.IsConfigurable = 0 and  rp.RelatedProductID is not null and pdConfi.ProductID is not null)
						)
					)	 
				 )
				 and 
				 (
				 --check for Be The First product. This logic should be migrated to the mgm business rules
					isnull(cast(pavBtf.Value as bit), 0) = 1 or
					(vsettings.Value is null or ( vsettings.Value = 'TRUE' and ( vstock.QuantityOnHand > 0 or p.IsConfigurable = 1)))
				 )
				 and (vsettingsWebshop.Value is null or ( vsettingsWebshop.Value = 'TRUE' and ( vsW.QuantityOnHand > 0 or p.IsConfigurable = 1)))
			 ) 
		)		
