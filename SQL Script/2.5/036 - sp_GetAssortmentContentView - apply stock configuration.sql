...
/****** Object:  StoredProcedure [dbo].[GetAssortmentContentView]    Script Date: 1-8-2013 15:15:57 ******/
SET ANSI_NULLS ON
GO
	...
	WHERE C.ConnectorID = @ConnectorID
		...
		(
		 (p.isconfigurable = 1 or (p.IsConfigurable = 0 and rp.ProductID is null) or(p.IsConfigurable = 0 and rp.ProductID is not null and cConfi.ProductID is not null)) and
			(ct.IgnoreMissingImage = 1 or (ct.IgnoreMissingImage = 1 and pm.imageCount > 0 and (rp.productid is null or (rp.productid is not null and cpm.imageCount > 0)))) and
