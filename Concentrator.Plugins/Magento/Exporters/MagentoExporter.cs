...
using Concentrator.Plugins.Magento.Contracts;
using Concentrator.Plugins.Magento.Providers;
...
namespace Concentrator.Plugins.Magento.Exporters
{
  public class MagentoExporter : BaseExporter
  {
    ...
    private List<AssortmentProductInfo> AssortmentToDeactivate;
    ...
    /// <summary>
    /// If set to false, export will not parse ConcentratorValue as int but will leave it to string. Used whenever attribute values are not proprietary codes. E.g. color code vs color name
    /// </summary>
    private bool TreatConcentratorAttributeValueAsNumber { get; set; }
    private const string TREAT_CONCENTRATOR_ATTRIBUTE_VALUE_AS_NUMBERSETTINGKEY = "Magento.TreatConcentratroValueIDAsNumber";
    ...
    private IMediaExchangeProvider mediaProvider;
      ...
      : base(connector, logger)
    {
      ...
      TreatConcentratorAttributeValueAsNumber = Connector.ConnectorSettings.GetValueByKey<bool>(TREAT_CONCENTRATOR_ATTRIBUTE_VALUE_AS_NUMBERSETTINGKEY, true);
      ...
      mediaProvider = (IMediaExchangeProvider)Activator.CreateInstance(mediaExchangeProviderType, Connector);
    }
    ...
    protected override void Process()
    {
      ...
      ProductDiffComparisonHelper productComparisonHelper = null;
      ...
      foreach (var language in Languages)
      {
...
#if CACHE
        var cacheDir = Path.Combine(_serializationPath, "CACHE", Connector.ConnectorID.ToString(), language.LanguageID.ToString());
        if (!Directory.Exists(cacheDir))
        {
          Directory.CreateDirectory(cacheDir);
        }

        var assortmentCacheFile = Path.Combine(cacheDir, "assortment.xml");
        var attributesCacheFile = Path.Combine(cacheDir, "attributes.xml");
        var contentCacheFile = Path.Combine(cacheDir, "content.xml");

        //assortment
        if (File.Exists(assortmentCacheFile))
        {
          Assortment = File.ReadAllText(assortmentCacheFile).FromXml<List<AssortmentProductInfo>>();
        }
        else
        {
          ...
          File.WriteAllText(assortmentCacheFile, Assortment.ToXml());
        }


        if (File.Exists(attributesCacheFile))
        {
          AttributesXml = XDocument.Load(attributesCacheFile);
        }
        else
        {
          var Soap = new AssortmentService();
          AttributesXml = XDocument.Parse(Soap.GetAttributesAssortmentByLanguage(Connector.ConnectorID, null, null, language.LanguageID).OuterXml);
          File.WriteAllText(attributesCacheFile, AttributesXml.ToString());
        }

        if (File.Exists(contentCacheFile))
        {
          ContentXml = XDocument.Load(contentCacheFile);
        }
        else
        {
          var Soap = new AssortmentService();
          var contentXml = Soap.GetAssortmentContentDescriptionsByLanguage(Connector.ConnectorID, null, language.LanguageID);
          using (XmlNodeReader read = new XmlNodeReader(contentXml))
          {
            ContentXml = XDocument.Load(read);
          }

          File.WriteAllText(contentCacheFile, ContentXml.ToString());
        }
        ...
        XDocument attributesXml = null;
        attributesXml = XDocument.Parse(Soap.GetAttributesAssortmentByLanguage(Connector.ConnectorID, null, null, language.LanguageID).OuterXml);

        XmlDocument contentXml = null;
        contentXml = Soap.GetAssortmentContentDescriptionsByLanguage(Connector.ConnectorID, null, language.LanguageID); //.OuterXml;       
        ...
        productComparisonHelper = new ProductDiffComparisonHelper(Assortment, ContentXml, AttributesXml, serializationPath);

        Assortment = productComparisonHelper.ProductsToProcess;
        AssortmentToDeactivate = productComparisonHelper.ProductsToDelete;
        ...
        if (UsesConfigurableProducts)
        {
          using (var helper = new AssortmentHelper(Connector.Connection, Version))
          {
            ...
            int storeid = StoreList[CurrentStoreCode].store_id;
            ...
            foreach (var avg in AttributeValueGroups)
            {
              ...
              if (!IsChildConnector && IsPrimaryLanguage)
              {
                foreach (var avg_value in avg.Value)
                {
                  var x = helper.GetOptionId(
        ...
        productComparisonHelper.ArchiveExportedAssortment(); //save the last exportedAssortment
      }

      if (productComparisonHelper == null)
        return;

      var mappingIds = (from pg in productComparisonHelper.CurrentFullAssortment.SelectMany(c => c.ProductGroups)
    ...
    private void SyncProducts()
    {
      ...
      if (productsSource.Count() > 0)
      {
        ...
        if (UsesConfigurableProducts)
        {
          ...
          var basePath = Connector.ConnectorSettings.GetValueByKey<string>("FtpPath", string.Empty).Replace("/catalog/product", "");
              ...
              foreach (var groupedAttribute in groups)
              {
                ...
                foreach (var attributeValueGroup in groupedAttribute)
                {
                  ...
                  if (options.ContainsKey(attribute_option_key))
                  {
                    ...
                    DownloadImage(new Uri(attributeValueGroup.Attribute("Image").Value),
        ...
        if (RelatedProducts != null)
        {
          using (var helper = new AssortmentHelper(Connector.Connection, Version))
          {
            ...
            foreach (var rp in RelatedProducts)
            {
              ...
              {
                if (!skuList.ContainsKey(child.RelatedProductManufacturerID)) continue;
      ...
      if (AssortmentToDeactivate == null && productsSource.Any())
      {
    ...
    /// <summary>
    /// Deactivates the specified assortment
    /// </summary>
    /// <param name="productsSource"></param>
    private void DeactivateAssortmentPartial(IEnumerable<AssortmentProductInfo> productsSource)
    {
      using (var helper = new AssortmentHelper(Connector.Connection, Version))
      {
        var completeSkuList = helper.GetActiveSkus();

        var toDeactivate = (from p in productsSource
                            let entity_id = (completeSkuList.ContainsKey(p.ManufacturerID) ? completeSkuList[p.ManufacturerID] : 0)
    ...
    /// <summary>
    /// Checks all products which are active in magento and not in the source
    /// Deactivates them
    /// </summary>
    /// <param name="productsSource"></param>
    private void CheckFullAssortmentAndDeactivateProducts(IEnumerable<AssortmentProductInfo> productsSource)
    {
      #region Deactivating
      ...
      )
    {
      ...
      if ((specialPrice ?? 0) > 0)
      {
        ...
        if (Connector.ConnectorID == 11 || Connector.ParentConnectorID == 11) //fix it in a different way
        {
      ...
      if (IsPrimaryLanguage)
      {
        ...
        if (RequiresStockUpdate || SyncStockForChildConnector)
        {
          ...
          helper.SyncStock(new cataloginventory_stock_item()
          {
            ...
            is_in_stock = is_in_stock || product.PresaleProduct
          });

          if (!is_in_stock && !product.PresaleProduct)
      ...
      if (productAttributes != null)
      {
        foreach (var attribute in productAttributes.Attributes)
        {
          ...
          string concentrator_attribute_value = null;

          if (!TreatConcentratorAttributeValueAsNumber)
          {
            concentrator_attribute_value = attribute.Attribute("AttributeOriginalValue").Value;
          }
          else
          {
            if (attribute.Attribute("AttributeOriginalValue") != null)
            {
              ...
              if (Int32.TryParse(attribute.Attribute("AttributeOriginalValue").Value, out x))
                concentrator_attribute_value = x.ToString();
          ...
          concentrator_attribute_value_id: concentrator_attribute_value,
          ...
          if (isAlsoGrouped)
          {
            ...
            if (Connector.ConnectorID == 11 || (!Connector.ParentConnectorID.HasValue && IsPrimaryLanguage))
            {
    ...
    void DownloadImage(Uri source, string targetFile, IAuditLogAdapter Logger)
    {
      try
      {
        ...
        {
#if DEBUG
          source = new Uri(source.AbsoluteUri.Replace("localhost", "54.72.137.147").Replace("ProductGroup", "Mapping"));
          ...
          {
            mediaProvider.Upload(stream, targetFile);
    ...
    public class ProductGroupInfo
    {
      ...
      public ProductGroupInfo()
      {
        ...
        AssortmentSeoTexts = new List<AssortmentSeoTexts>();
      }
    ...
    private void ExportCategories()
    {
      ...
      SyncCategory(currentRoot, roots.Select(x => x.Value).ToList(), helper, defaultAttributeSet.attribute_set_id, attributeList, currentCategories);

      helper.UpdateCategoryChildCount();
    }

    private void SyncCategory(catalog_category_entity parent, IEnumerable<ProductGroupInfo> categories, AssortmentHelper helper, int attribute_set_id, SortedDictionary<string, eav_attribute> attributeList, List<catalog_category_entity> currentCategories)
    {
      if (parent.level >= 6)
        return;
      ...
      foreach (var cat in categories)
      {
        ...
        //seo texts
        foreach (var seoDescriptionType in Enum.GetValues(typeof(SeoDescriptionTypes)))
        {
          string seoDescriptionTypeName = seoDescriptionType.ToString().ToLower();

          if (!attributeList.ContainsKey(seoDescriptionTypeName)) continue;

          var currentSeo = cat.AssortmentSeoTexts.FirstOrDefault(x => x.DescriptionType == (int)seoDescriptionType);

          if (currentSeo != null && !string.IsNullOrEmpty(currentSeo.Description))//update seo description from DB with currentSeo
          {
            helper.SyncAttributeValue(attributeList[seoDescriptionTypeName].attribute_id, CATEGORY_ENTITY_TYPE_ID, store_id_actual, entity.entity_id, currentSeo.Description, type: "text");

            if (IsPrimaryLanguage)
              helper.SyncAttributeValue(attributeList[seoDescriptionTypeName].attribute_id, CATEGORY_ENTITY_TYPE_ID, 0, entity.entity_id, currentSeo.Description, type: "text");

          }
          else //delete current seo description from DB
          {
            helper.DeleteAttributeValue(attributeList[seoDescriptionTypeName].attribute_id, CATEGORY_ENTITY_TYPE_ID, store_id_actual, entity.entity_id, type: "text");
            if (IsPrimaryLanguage)
              helper.DeleteAttributeValue(attributeList[seoDescriptionTypeName].attribute_id, CATEGORY_ENTITY_TYPE_ID, 0, entity.entity_id, type: "text");
          }
        }
        ...
        if (cat.Image != null)
        {
          ...
          {
            //var ftpInfo = SftpHelper.GetFtpInfo(Connector);
            var basePath = Connector.ConnectorSettings.GetValueByKey<string>("FtpPath", string.Empty).Replace("/catalog/product", "/catalog/category");
            if (!basePath.EndsWith("/"))
              basePath += "/";
            ...
            DownloadImage(new Uri(cat.Image), basePath + fileName, Logger);
        ...
        if (cat.ThumbnailImage != null)
        {
          ...
          else
          {
            ...
            fileName = HttpUtility.UrlDecode(fileName);
            DownloadImage(new Uri(cat.ThumbnailImage), basePath + fileName, Logger);
        ...
        if (cat.MenuIconImage != null && attributeList.ContainsKey("menuicon"))
        {
          if (cat.MenuIconImage == String.Empty)
            helper.DeleteAttributeValue(attributeList["menuicon"].attribute_id, CATEGORY_ENTITY_TYPE_ID, store_id, entity.entity_id, type: "varchar");
          else
          {
            var basePath = Connector.ConnectorSettings.GetValueByKey<string>("FtpPath", string.Empty).Replace("/catalog/product", "/catalog/category");
            if (!basePath.EndsWith("/"))
            ...
            Uri source = new Uri(cat.MenuIconImage);

            string fileName = source.Segments.Last();

            fileName = HttpUtility.UrlDecode(fileName);
            DownloadImage(new Uri(cat.MenuIconImage), basePath + fileName, Logger);
            helper.SyncAttributeValue(attributeList["menuicon"].attribute_id, CATEGORY_ENTITY_TYPE_ID, store_id_actual, entity.entity_id, fileName, type: "varchar");
            if (IsPrimaryLanguage)
              helper.SyncAttributeValue(attributeList["menuicon"].attribute_id, CATEGORY_ENTITY_TYPE_ID, 0, entity.entity_id, fileName, type: "varchar");
          }
        }
        else
        {
        ...
        foreach (var setting in cat.ProductGroupSettings)
        {
          if (attributeList.ContainsKey(setting.Name))
          {
            helper.SyncAttributeValue(attributeList[setting.Name].attribute_id, CATEGORY_ENTITY_TYPE_ID, store_id, entity.entity_id, setting.Value);
            if (IsPrimaryLanguage)
              helper.SyncAttributeValue(attributeList[setting.Name].attribute_id, CATEGORY_ENTITY_TYPE_ID, 0, entity.entity_id, setting.Value);
          }
          else
          {
            var attribute = helper.CreateAttribute(setting.Name, CATEGORY_ENTITY_TYPE_ID, setting.Name);
            var newAttributeID = helper.SyncAttribute(attribute, true);

            helper.SyncAttributeValue(newAttributeID, CATEGORY_ENTITY_TYPE_ID, store_id, entity.entity_id, setting.Value);
            if (IsPrimaryLanguage)
              helper.SyncAttributeValue(newAttributeID, CATEGORY_ENTITY_TYPE_ID, 0, entity.entity_id, setting.Value);
          }
        }
        ...
        if (cat.Children.Count > 0)
        {
          ...
          SyncCategory(entity, cat.Children, helper, attribute_set_id, attributeList, currentCategories);
        }
      }
    }
    ...
    private ProductGroupInfo BuildCategoryTree(AssortmentProductGroup group, Dictionary<string, ProductGroupInfo> result, bool firstLevel = false)
    {
      ...
      string menuIconImage = (!String.IsNullOrEmpty(group.MenuIconImage)) ? group.MenuIconImage : null;
      ...
      if (!result.TryGetValue(mappingKey, out info))
      {
        info = new ProductGroupInfo()
        {
          ...
          AssortmentSeoTexts = group.AssortmentSeoTexts != null ? group.AssortmentSeoTexts : new List<AssortmentSeoTexts>(),
          ProductGroupSettings = group.ProductGroupSettings
        };
