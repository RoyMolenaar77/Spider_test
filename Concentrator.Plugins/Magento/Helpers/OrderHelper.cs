...
INNER join catalog_product_entity_decimal cp ON (cp.entity_id=catalog_product_entity.entity_id   AND sales_flat_order.store_id = cp.store_id
            ...
            INNER join core_store cs on cp.store_id = cs.store_id " +
             (string.IsNullOrEmpty(websiteCodeInCoreTable) ? "" : "INNER join core_website cw on cs.website_id = cw.website_id AND cw.code = '{3}'")
              +
            @"left join catalog_product_entity_decimal cp2 ON (cp2.entity_id=catalog_product_entity.entity_id  AND 0 = cp2.store_id
